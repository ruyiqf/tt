...
class TDXDLL:
    def __init__(self):
        ...
        #print(__file__)
        #print(dllpath)
        print(util.find_library('trader.dll'))
    ...
    def querydata(self, category):
        ...
        self.dll.QueryData(self.sessionid, category, result, errmsg)
        return (True, result.value.decode('gbk')) if errmsg.value == b"" else (False, errmsg.value.decode('gbk'))
    ...
    def sendorder(self, category, pricetype, gddm, zqdm, price, quantity):
        ...
        errmsg = create_string_buffer(256)
        result = create_string_buffer(1024 * 1024)
        self.dll.SendOrder(self.sessionid, category, pricetype, gddm.encode(), zqdm.encode(), price, quantity, result,
                           errmsg)
        return (True, result.value.decode('gbk')) if errmsg.value == b"" else (False, errmsg.value.decode('gbk'))
